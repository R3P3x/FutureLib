--[[
  This script is compatible with Roblox games! (intended for server side, tho if you put localplayer as the plr variable you can use local scripts)
]]

if not getfenv() then
  local NotificationLibrary = {}
  
  function NotificationLibrary:Notify(plr : Player, TitleText, Desc, Delay)
...
elseif getfenv() then
  ...
  local NotificationLibrary = {}
  function NotificationLibrary:Notify(plr, TitleText, Desc, Delay)
    error("Either this is implemented incorrectly into a Roblox game or your exploit does not support getfenv()! Because of this the library cannot load")
  end
  return NotificationLibrary
end
